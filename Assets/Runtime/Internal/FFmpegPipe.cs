...
namespace FFmpegOut
{
    public class FFmpegPipe : System.IDisposable
    {
        #region Public methods

        public static bool IsAvailable {
            get {
				return true;//System.IO.File.Exists(ExecutablePath); 
			}
        }

        public FFmpegPipe(string arguments) 
		: this(arguments, ExecutablePath) {}
        public FFmpegPipe(string arguments, string derech)
        {
       ...
       public void PushFrameData(NativeArray<byte> data)
		...
		public void PushFrameData(byte[] data)
        {
            // Update the copy queue and notify the copy thread with a ping.
          /*  lock (_pipeQueue) _pipeQueue.Enqueue(data);
            _pipePing.Set();*/
        }
