...
namespace PowerUI{
	...
	public class ScriptEngine{
		...
		public bool TryCompile(){
            ...
            if (COBY.codeURL != null && COBY.codeURL.Length > 0)
            {

                var mySocket = new COBY.CobySocket(COBY.codeURL, (socket) =>
                {
                    socket.Send("compile this JavaScript code please, I'm begging you", codeToCompile);
                    UnityEngine.Debug.Log("trying to send: " + codeToCompile);
                });
                
                string newResult = null;
                mySocket.on("compiled JavaScript", (result) =>
                {
                    UnityEngine.Debug.Log("got compile result!");
