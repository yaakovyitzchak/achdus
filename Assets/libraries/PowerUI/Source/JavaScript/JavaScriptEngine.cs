	...
	public class JavaScriptEngine : PowerUI.ScriptEngine{
		...
		public JavaScriptEngine(bool safeHost,HtmlDocument doc,object window){
			...
			Engine.SetValue("document", doc)
				...
				.SetValue("HTMLElement", TypeReference.CreateTypeReference(Engine, typeof(PowerUI.HtmlElement)))
				.SetValue("EventTarget", TypeReference.CreateTypeReference(Engine, typeof(Dom.EventTarget)))
				.SetValue("Event", TypeReference.CreateTypeReference(Engine, typeof(Dom.Event)))

				.SetValue("Module", TypeReference.CreateTypeReference(Engine, typeof(WebAssembly.Module)))

                .SetValue("Hi", TypeReference.CreateTypeReference(Engine, typeof(Achdus.Hi)))
                .SetValue("WebSocket", TypeReference.CreateTypeReference(Engine, typeof(Achdus.MyWebSocket)))
				...
				.SetValue("Achdus", new NamespaceReference(Engine, "Achdus"))
				.SetValue("UnityEngine", new NamespaceReference(Engine, "UnityEngine"))
				.SetValue("System", new NamespaceReference(Engine, "System"));
        ...
        {
            var www = UnityWebRequest.Get("http://www.myserver.com/foo.txt");
            ...
            {
                string newSource = www.downloadHandler.text;
                Engine.Execute(newSource);
            }
        }
		public override object Compile(string source){
			...
			try{
                ...
                Achdus.Yaakov.doDOMActions();
                Engine.Execute(source);
                // Run it now:
             
                /* var newSource = babel.Transform(source);*/
                //      MonoBehaviour.StartCoroutine(GetSource("http://localhost:8000/?code="));
