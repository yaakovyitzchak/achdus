	...
	private void EncodeAndSave()
	{
		...
		while (threadIsProcessing) 
		{
			if(frameQueue.Count > 0)
			{
				...
				{
					BitmapEncoder.WriteBitmap(fileStream, 1920, 1080, frameQueue.Dequeue());
					fileStream.Close();

					names.Add(path);
					timeStamps.Add(DateTime.Now.ToFileTime());
				}
		...
		var bw = new BinaryWriter(newStream);
		bw.Write(
